...
- [x] **6.4 七步仪式 Step 7 迷你卡 C4/C5/C6 形态** ✅ **代码完成（2026-07-21，远程纯代码场次）** — ⏳ 真机验收待做
  - ✅ **决策（用户拍板 2026-07-21）：加 mini-C4（推导），有意排除 C5/C6**——易混/题型需跨公式知识，
    首次编码偏早，交给复习（复习里 C5/C6 已有）。
  - ✅ **巩固 deck 空值驱动**：`FormulaLearnRitualViewModel.load` 按数据构建 `miniDeck`——
    C1 恒有 / C2 有 cloze / C3 有条件 / C4 有推导才进；去掉原 mini-C2 的"自动通过"占位。
  - ✅ **MiniC4Card**：回想推导 →「看推导」揭晓推导链（逐步 latex+note）→ 自评「推出来了/没推出来」
    （驱动 onPass/onFail 入重做队列，不算 FSRS 分，形态对齐 MiniC3）。
  - ✅ C5/C6 dispatch else 分支保留为防御兜底（deck 已不含它们）。
  - ✅ 编译过 + 全套单测绿。
  - ⏳ **真机验收待做**：起有推导的公式 → Step7 序列含 mini-C4 → 看推导→自评→并入结业；无推导公式序列不含 C4。
