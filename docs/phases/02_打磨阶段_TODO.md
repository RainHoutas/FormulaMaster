---
name: Formula Master — 打磨完善阶段 TODO（已归档）
description: 原型之后的精细化开发，Sprint 从改进点池滚动生成
created: 2026-04-24
archived: 2026-05-08
parentPhase: 01_原型阶段（已归档）
nextPhase: 03_学习流程重构阶段（进行中）
status: 已收尾
summary: docs/phases/summaries/打磨阶段总结.md
---

# 打磨完善阶段（已归档）

> 🗄️ **历史归档**。对接现状请读 [`../design/架构总览.md`](../design/架构总览.md)；文中对旧文档名的引用仅作溯源。

> ⚠️ **本阶段已于 2026-05-08 收尾，进入「学习流程重构阶段（03）」**。
> - 阶段总结：[`../打磨阶段总结.md`](summaries/打磨阶段总结.md)
> - 下一阶段 TODO：[`03_学习流程重构_TODO.md`](03_学习流程重构_TODO.md)
> - 下一阶段 RFC：[`../RFC_学习流程重构.md`](../planning/RFC_学习流程重构.md)
>
> 本阶段共完成 3 个 Sprint，230/230 单测全绿。详细产出见上方总结文档。
> 历史记录保留以下，便于回溯。

---

> 本阶段承接[原型阶段](01_原型阶段_TODO.md)，把 Mock 实现替换为真实可用，把"够用"的 UI 精修到"想用"。
> 与原型阶段不同，本阶段 Sprint 的内容由 [`../改进点池.md`](../planning/改进点池.md) 滚动生成。
...
Sprint 1 收尾后用户反馈了一组改进点（见 [`../改进点池.md`](../planning/改进点池.md)）。本 Sprint 取
TOP 5 条目落地：
...
- **OcrFeedback 数据模型重构**（用户 2026-05-01 拍板，新增）：原"手输正确 LaTeX"设计
  随 LaTeX 输入法搁置失效，**复用现有 `FormulaEntity.clozeData` 已拆好的 placeholder**
  作为 chip 多选项，让用户标记哪些部件被识别错（用户 2026-05-01 指出现有 clozeData
  天然提供拆分粒度，无需新写 LatexTokenizer）
- ~~Test 移出 NavBar~~：用户决定保留 NavBar 一致性（2026-05-01），已移入「已拒绝/搁置」

### 关键决策记录

- **LaTeX 输入法**：无限期搁置，等"质量过硬的开源 Compose LaTeX 输入面板"或"纸笔自评
  局限暴露明显"才重启
- **OcrFeedback schema 升级**：加 `wrongPlaceholdersJson` 字段（用户从 clozeData
  placeholder 多选的部件列表），`correctLatex` 改 nullable 兼容旧导出；`AppDatabase` v2 → v3，
  沿用 `fallbackToDestructiveMigration(dropAllTables=true)`（打磨阶段允许）
- **复用 ClozeParser**：不新写 tokenizer，FeedbackDialog 直接渲染 `ClozeItem.placeholder` 列表
- **clozeData 空数组**边界：保留「都不对」按钮兜底
- **Test 保留在 NavBar**：用户拍板，"Test 移出 NavBar"改进点已拒绝
- **Sprint 3 收尾后**：进入 P0「学习与复习流程重构」task

### Task 列表

- [x] **Task 3.1** InputMode 基础设施 ✅ 2026-05-01
  - `domain/InputMode.kt` 枚举（`Handwriting` / `PaperPen`）+ displayName + description
  - `AppPreference.AppSettings.inputMode: InputMode = Default`，加 `KEY_INPUT_MODE` (string)
  - `setInputMode(mode)` setter；`toInputModeOrDefault()` 兜底未知值
  - `SettingsViewModel.setInputMode` 转发
  - `SettingsScreen` 新增「输入偏好」区，ExposedDropdown 二选一带 description；
    选 PaperPen 时提示"进入严测时会自动切换横屏"
  - **Done 标准**：BUILD SUCCESSFUL + 既有单测无回归 ✓
...
- [x] **Task 3.2** 纸笔自评模式 PaperPenInputArea ✅ 2026-05-06
  - 新建 `ui/component/PaperPenInputArea.kt`：
    - **屏蔽所有手写相关 UI**：不挂 TestCanvas、预览条、候选 chip、「都不对」按钮、强识别按钮
    - 公式以 `MathFormulaView(fillMaxSize)` 最大化显示
    - 大按钮"已完成默写"
    - 点击后展开标准答案 + 自评按钮「完全正确」/「出现错误」
  - `TestScreen` 按 `appSettings.inputMode` 路由：
    - `Handwriting` → 现有 TestCanvas 路径
    - `PaperPen` → `PaperPenInputArea` 路径（不调识别器，不消耗额度）
  - **强制横屏**（用户追加要求，2026-05-01）：
    - 进入 TestScreen 且 InputMode=PaperPen 时 `requestedOrientation = SCREEN_ORIENTATION_SENSOR_LANDSCAPE`
    - 退出 TestScreen 或切回手写模式时恢复 `SCREEN_ORIENTATION_UNSPECIFIED`
    - 用 `DisposableEffect(inputMode)` 在 Composable 生命周期内管理
  - 自评结果调 `viewModel.submitJudgment(item, isCorrect, costTimeMs)`，与现有路径对齐
  - **Done 标准**：BUILD SUCCESSFUL；真机切到纸笔模式不再调用识别器（Logcat 验证）；
    自评后正确进入下一题；进入纸笔 Test 自动转横屏，退出恢复
  - **实施摘要**（2026-05-06）：
    - 两阶段交互：Phase 1 隐藏答案 + 居中提示 + 全宽"已完成默写"按钮 → Phase 2 标准答案最大化 + 横排自评按钮
    - 中部弹性区用 `Box(weight(1f))` + 内层 Column scope 内 `MathFormulaView.weight(1f)` 跨 scope 嵌套
    - `DisposableEffect(appSettings.inputMode)` 放 TestScreen 顶层，mode 切换 / 离开 TestScreen 时 `onDispose` 恢复
    - 已知小限制：`isDone` 用 `remember` 而非 `rememberSaveable`，主动 180° 翻转设备会重置回 Phase 1（边缘场景）

- [x] **Task 3.3** Onboarding 加"输入方式"页 ✅ 2026-05-06
  ...
  - **实施摘要**（2026-05-06）：
    - 5 页 → 6 页；新页插在 page 1（欢迎之后），原 page 1-4 后移到 2-5
    - 默认高亮"手写识别"；选纸笔时下方显示蓝色提示"已选纸笔自评，下一步将跳过识别器配置"
    - 双向跳过逻辑：page 3 → next 时若 PaperPen → 跳到 page 5；page 5 → prev 时若 PaperPen → 回到 page 3
    - "跳过引导"条件 `< 4` → `< 5`；最后页判定 `== 4` → `== 5`；PageIndicator count `5` → `6`

- [x] **Task 3.4** OcrFeedback 数据模型重构 🔥 ✅ 2026-05-06
  - 数据层：
    - `OcrFeedbackEntity` 加 `wrongPlaceholdersJson: String?` 字段（用户从 placeholder
      多选的子部件列表的 JSON）；`correctLatex` 改 nullable 保留
    - `AppDatabase` v2 → v3，`fallbackToDestructiveMigration(dropAllTables=true)`
  - 公式部件来源：
    - **完全复用 `domain/ClozeParser.parse(formula.clozeData)`** 拿到 `ClozeItem` 列表
    - 每个 `ClozeItem.placeholder` 即一个可选错位的 LaTeX 子部件
    - 不新建 tokenizer
  - UI 层：
    - `FeedbackDialog` 替换"输入正确 LaTeX"为：
      - 上半部：笔画预览 + 候选 chip 列表（保留）
      - 中部：当前公式渲染 (MathFormulaView) 让用户对照
      - 下半部：FlowRow + FilterChip 显示 placeholder 列表（每个渲染成 LaTeX），多选高亮
      - 「标记错误部件」按钮（多选确认后提交）+「都不对」快捷 = 全选所有 placeholder
    - clozeData 空数组的边界场景：禁用 chip 多选区，保留「都不对」单一选项
    - `TestViewModel.submitOcrFeedback` 接收 `wrongPlaceholders: List<String>` 替代 `correctLatex`
  - 导出 JSON 格式同步更新（schema 演进）
  - **Done 标准**：
    - BUILD SUCCESSFUL；既有单测无回归
    - 真机：进入反馈 → 选 placeholder chip → 提交 → 设置页样本数 +1 → 导出 JSON 含新字段
    - clozeData 空的公式仍可提交「都不对」反馈
  - **实施摘要**（2026-05-06，多轮迭代）：
    ...
    - 新组件 `LatexChipsView`（通用、`selectable: Boolean`）+ 模板 `latex_chips_template.html`：
      单 WebView 一次性渲染所有 chip + KaTeX + JS bridge 双向同步选中态 + ResizeObserver 高度自适应
    - 候选区（`selectable=false`，filled 风格）+ 多选区（`selectable=true`，outlined 风格）共用此组件，M3 视觉差异明显
    - `FeedbackDialog` 容器从 `AlertDialog` 换为 `Dialog + Surface(heightIn(max = 88% 屏高))`
      + `Column.weight(1f, fill = false) + verticalScroll`，内容少时按需收缩，超上限滚动
    - **去掉「都不对」按钮**（用户反馈基本用不到）：兜底语义合并到主按钮
      - 有 placeholders + 无选中 → "标记错误部件" 禁用
      - 有 placeholders + 有选中 → "标记错误部件" 提交所选
      - 无 placeholders（clozeData 空数组）→ "提交反馈" 永远可点，提交空列表（语义：整体失败）
  - **关键踩坑**：
    1. JS 上报的 `body.scrollHeight` 是 **CSS px**（在 `useWideViewPort=false` 下 = dp），
       Kotlin 不要再走 `with(density){px.toDp()}` 转换（会被除掉 density 倍 → chip 区被裁切显半截）
    2. KaTeX 字体异步加载完后 chip 高度变化要靠 `ResizeObserver` 持续监听 + `document.fonts.ready` 双保险
    3. `Surface.fillMaxHeight(0.88f)` + `Column.weight(1f)+verticalScroll` 会强制撑满高度造成下方大片空白；
       改 `heightIn(max = 88%)` + `weight(1f, fill = false)` 让 dialog 按内容收缩
...
- 真机端到端：
  ...
  - Onboarding 包含输入方式选择
  - 反馈 Dialog 用 placeholder 多选替代 LaTeX 输入框
...
### Sprint 3 总结（2026-05-01 ~ 2026-05-06）

完成输入方式二选一 + 反馈机制重构：

- **Task 3.1**：`InputMode` 枚举 + `AppPreference.inputMode` 持久化 + 设置页"输入偏好"下拉
- **Task 3.2**：纸笔自评模式 `PaperPenInputArea` 落地，两阶段交互（隐藏答案默写 → 标准答案核对自评）；
  `TestScreen` 按 `appSettings.inputMode` 路由；`DisposableEffect` 强制横屏 + 离开恢复
- **Task 3.3**：Onboarding 5 页 → 6 页，第 2 页让用户选输入方式（手写识别 / 纸笔自评），
  选纸笔时双向跳过识别器配置页
- **Task 3.4**：`OcrFeedback` schema 升级（DB v2→v3），用户从 `clozeData` 解析的 placeholder 列表
  多选错误部件，替代原"手输正确 LaTeX"；新增通用 `LatexChipsView` 组件 + `latex_chips_template.html`
  HTML 模板，单 WebView 批量渲染 KaTeX + JS bridge 双向桥；`FeedbackDialog` 全面重构
  （Dialog + Surface 替代 AlertDialog 给 chip 区充足空间，M3 两种 chip 风格区分候选/多选）

**Sprint 完成时**：`./gradlew.bat compileDebugKotlin testDebugUnitTest` BUILD SUCCESSFUL，230/230 单测全绿。
真机 UI 经过多轮用户截图反馈迭代修复（chip 区裁切 / 下方空白 / 候选源码渲染 / 视觉区分不足等），
最终全部通过用户验收。

**新踩坑入册**（详见 HANDOFF 注意事项段）：
1. JS `body.scrollHeight` 是 CSS px（≈ dp），Kotlin 不要再走 density 二次转换
2. WebView 内容高度自适应必须用 `ResizeObserver` 监听 KaTeX 字体异步加载触发的 reflow
3. Dialog 自适应内容高度：`Surface.heightIn(max=...)` + `Column.weight(1f, fill = false)` 而非 `fillMaxHeight`
4. AlertDialog 多按钮被强制塞同一行，超过 2 个会折行；3 按钮场景应改用自定义 Dialog
5. `WebView.addJavascriptInterface` 双向桥可行但 bridge 方法在 WebView 后台线程，
   写 Compose state 必须 `webView.post` 切回主线程

---

## Sprint 3 收尾后下一步（2026-05-06）

进入 P0「**学习与复习流程重构**」task —— 改进点池中标注为 P0 但范围未细化。
按方法论 #3，**先 RFC 讨论用户痛点不直接动手**：
...
新对话开启时由用户列出当前流程痛点 → 形成 RFC 文档 `docs/RFC_学习流程重构.md` →
拆分为可独立提交的子 Task 进入 Sprint 4。
