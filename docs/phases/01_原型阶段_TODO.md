...
> ⚠️ **对接现状勿读此文**：文中引用的 `Project_Spec.md` / `UI设计规范.md` / `核心数据库和算法设计.md` 已重命名并重写，当前架构权威见 [`../design/`](../design/)（尤其 [`架构总览.md`](../design/架构总览.md)）；下方原文保留原始引用仅作历史溯源。
> 当前进行中的活文件：[`../phases/02_打磨阶段_TODO.md`](02_打磨阶段_TODO.md)
> 阶段性总结：[`../原型阶段总结.md`](summaries/原型阶段总结.md)
>
> ---
>
