...
### 3.5 编码阶段六步学习仪式（用户拍板修订，2026-05-08）
...
### 9.2 后续阻塞（Sprint 2-4 启动前答即可）

- **D12 C5 易混对来源**（Sprint 3 阻塞）：手工编辑 / 教研整理 / AI 生成
- **D13 C6 题面来源**（Sprint 3 阻塞）：考研真题（版权）/ 自编简化题
- **D14 公式族图谱边类型**（Sprint 4 阻塞）：仅推出 / +易混 / +题型共享
- **D15 学习阶段切换是否允许回退**（Sprint 4 阻塞）：是 / 否
