# 考研数学公式记忆助手 (Formula Master) — 唯一执行计划 v3.0
...
- [ ] 所有卡片使用 `ElevatedCard` + `RoundedCornerShape(16.dp)`，无普通 `Card` 混用
- [ ] 所有高频操作按钮使用 `FilledTonalButton`，非破坏性操作无实心高亮按钮
- [ ] 所有页面在深色模式下对比度合理，KaTeX 公式字体在深色背景下可见
- [ ] 所有页面开启 Edge-to-Edge 后，内容不被状态栏或导航栏遮挡
- [ ] 底部导航栏 Tab 切换有椭圆形选中高亮效果（M3 `NavigationBarItem` 默认样式）
- [ ] 页面横向切换有 `slideInHorizontally` / `slideOutHorizontally` 动画
- [ ] 所有按钮点击均有水波纹（Ripple）效果，无裸 `clickable` 无反馈交互
- [ ] 测试 Tab 进入/退出后底部导航栏正确隐藏/恢复
- [ ] `errorContainer` 颜色仅用于错误/危险场景，不滥用
